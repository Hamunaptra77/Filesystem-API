...
import json
import secrets
from config import ALLOWED_DIRECTORIES, ALLOWED_ORIGINS, FILESYSTEM_API_KEY, DOMAIN
...
def get_api_key(api_key_header: str = Security(api_key_header)) -> APIKey:
    if FILESYSTEM_API_KEY is None:
        return APIKey("none")
    if api_key_header == FILESYSTEM_API_KEY:
        return APIKey(api_key_header)
    raise HTTPException(
        status_code=401,
...
app = FastAPI(
    title=f"Secure Filesystem API ({DOMAIN})",
    version="2.0.0",
    ...
    dependencies=[Security(get_api_key)] if FILESYSTEM_API_KEY else [],
)
...
class DirectoryEntry(BaseModel):
    ...
    type: Literal["file", "directory"] = Field(..., description="Type of entry")
...
class DirectoryTreeNode(BaseModel):
    name: str = Field(..., description="Name of the file or directory")
    type: Literal["file", "directory"] = Field(..., description="Type of entry")
    children: Optional[List['DirectoryTreeNode']] = Field(default=None, description="Child entries for directories")
...
class DirectoryTreeResponse(BaseModel):
    tree: List[DirectoryTreeNode] = Field(..., description="Recursive tree structure")


class FileSearchMatch(BaseModel):
    path: str = Field(..., description="Full path to matched file")


class SearchFilesResponse(BaseModel):
    matches: List[FileSearchMatch] = Field(..., description="List of files matching the pattern")
...
class SearchContentResponse(BaseModel):
    matches: List[ContentSearchMatch] = Field(..., description="List of content matches")


class MetadataResponse(BaseModel):
    path: str = Field(..., description="Absolute path to the file or directory")
    type: Literal["file", "directory", "other"] = Field(..., description="Type of path")
    size_bytes: int = Field(..., description="Size in bytes")
    modification_time_utc: str = Field(..., description="Last modification time in ISO format (UTC)")
    creation_time_utc: str = Field(..., description="Creation/birth time in ISO format (UTC)")
    last_metadata_change_time_utc: str = Field(..., description="Last metadata change time in ISO format (UTC)")
...
@app.post(
    "/list_directory", response_model=DirectoryListingResponse, summary="List a directory"
)
async def list_directory(data: ListDirectoryRequest = Body(...)):
    ...
    try:
        ...
        for entry in dir_path.iterdir():
            ...
            listing.append(DirectoryEntry(name=entry.name, type=entry_type))

        return DirectoryListingResponse(entries=listing)
    except HTTPException:
        raise
    except PermissionError:
        raise HTTPException(status_code=403, detail=f"Permission denied to list directory {data.path}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to list directory {data.path}: {str(e)}")


@app.post("/directory_tree", response_model=DirectoryTreeResponse, summary="Recursive directory tree")
async def directory_tree(data: DirectoryTreeRequest = Body(...)):
    ...
    try:
        ...
        def build_tree(current: pathlib.Path) -> List[DirectoryTreeNode]:
            entries = []
            try:
        ...
        raise HTTPException(status_code=403, detail=f"Permission denied to access {data.path}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to build directory tree for {data.path}: {str(e)}")


@app.post("/search_files", response_model=SearchFilesResponse, summary="Search for files")
            ...
            for item in files + dirs:
                if data.pattern.lower() in item.lower():
                    ...
                    if any(str(result_path).startswith(alt) for alt in ALLOWED_DIRECTORIES):
                        results.append(FileSearchMatch(path=str(result_path)))

        return SearchFilesResponse(matches=results)
    except HTTPException:
        raise
    except PermissionError:
        raise HTTPException(status_code=403, detail=f"Permission denied to search in {data.path}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Search failed for {data.path}: {str(e)}")
...
@app.post("/get_metadata", response_model=MetadataResponse, summary="Get file or directory metadata")
...
async def search_content(data: SearchContentRequest = Body(...)):
    ...
    try:
        ...
        return SearchContentResponse(matches=results)
    except HTTPException:
        raise
    except PermissionError:
        raise HTTPException(status_code=403, detail=f"Permission denied to search in {data.path}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Content search failed for {data.path}: {str(e)}")
...
@app.get("/health", summary="Filesystem API health check")
async def health_check():
    """
    Simple health check endpoint used by Docker and orchestrators.
    """
    return {"status": "healthy", "service": "filesystem-api"}
